...
JY_BASE_MODEL_DIR = os.path.join(ROOT_DIR, 'model', 'jy_base')
JY_LOG_MODEL_DIR = os.path.join(ROOT_DIR, 'model', "jy_log")
JY_MEMBER_MODEL_DIR = os.path.join(ROOT_DIR, 'model', "jy_member")
JY_OTHER_MODEL_DIR = os.path.join(ROOT_DIR, 'model', "jy_other")
JY_TRADE_MODEL_DIR = os.path.join(ROOT_DIR, 'model', "jy_trade")


# MODEL_DIRS = [JY_BASE_MODEL_DIR, JY_LOG_MODEL_DIR, JY_MEMBER_MODEL_DIR, JY_OTHER_MODEL_DIR, JY_TRADE_MODEL_DIR]
MODEL_DIRS = {"JYBaseDB": JY_BASE_MODEL_DIR,
              "JYLogDB": JY_LOG_MODEL_DIR,
              "JYMemberDB": JY_MEMBER_MODEL_DIR,
              "JYOtherDB": JY_OTHER_MODEL_DIR,
              "JYTradeDB": JY_TRADE_MODEL_DIR}
...
CONFIG_FILE = os.path.join(ROOT_DIR, "tools/config.json")
...
def render(conn, db_name, db_map, model_dir, package_name, is_base_db):
    ...
    for row in result:
        ...
        select_all_sql = 'select %s from %s' % (
            ','.join(['`' + c['COLUMN_NAME'] + '`'
                      for c in columns ]),
            table_name
        )
        select_where_sql = 'select %s from %s where 1=1 ' % (
            ','.join(['`' + c['COLUMN_NAME'] + '`'
                      for c in columns ]),
            table_name
        )
        get_by_pk_sql = 'select %s from %s where %s ' % (
            ','.join(['`' + c['COLUMN_NAME'] + '`'
                      for c in columns ]),
            table_name, ' and '.join(['%s =:%s' % (k, k) for k in primary_key]))
        get_by_pk_sql2 = 'select %s from %s where %s ' % (
            ','.join(['`' + c['COLUMN_NAME'] + '`'
                      for c in columns ]),
        ...
        render_dict = {
            ...
            "select_where_sql": select_where_sql,
        }
...
def run():
    ...
    for k, v in config.items():
        conn = pymysql.connect(host=v['Host'],
                               port=v['Port'],
                               charset='utf8',
                               user=v['User'],
                               passwd=v['Pass'],
        ...
        render(conn, v['Name'], k, MODEL_DIRS[k], k, k == "JYBaseDB")
